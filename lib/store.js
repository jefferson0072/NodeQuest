...
// Providers report their GPU + wallet. WE assign the tier — never the user.
export async function createProvider({ name, gpuModel, vramGb, wallet }) {
  const db = await kv();
  const pid = await nextId("prov");
  const assigned = assignTier({ gpuModel, vramGb });
  const provider = {
    id: pid,
    name,
    gpuModel: gpuModel || "Unknown GPU",
...
// Re-attach an existing provider with the same wallet + name (agent restarts),
// otherwise create a new one. Keeps reputation/earnings across reconnects.
export async function upsertProvider({ name, gpuModel, vramGb, wallet }) {
  if (wallet) {
    const all = await listProviders();
    const existing = all.find((p) => p.wallet === wallet && p.name === name);
    if (existing) {
      const assigned = assignTier({ gpuModel, vramGb });
      const updated = {
        ...existing,
        gpuModel: gpuModel || existing.gpuModel,
        vramGb: assigned.vramGb || existing.vramGb,
        tier: assigned.tier,
        tierSource: assigned.source,
        online: true,
      };
  ...
  return createProvider({ name, gpuModel, vramGb, wallet });
}
...
export async function createJob({ id, title, workload, workloadLabel, input, tier, reward, poster, deadlineSec }) {
  const db = await kv();
  const jid = id;
  const job = {
    id: jid,
