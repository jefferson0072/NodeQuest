...
export const BURN_RATE = 0.2; // 20% of every bounty is burned
export const PLATFORM_FEE_RATE = 0.03; // 3% platform fee
...
export async function settleJob(jobId) {
  ...
  // 4. Split the bounty: burn 20%, platform fee 3%, rest to winner.
  const burn = +(job.reward * BURN_RATE).toFixed(6);
  const fee = +(job.reward * PLATFORM_FEE_RATE).toFixed(6);
  const payout = +(job.reward - burn - fee).toFixed(6);
