...
// Match an nvidia-smi name to a catalog key (longest match first).
export function matchGpuCatalog(rawName) {
  if (!rawName) return null;
  const upper = rawName.toUpperCase();
  const keys = Object.keys(GPU_CATALOG).sort((a, b) => b.length - a.length);
  return keys.find((key) => upper.includes(key.toUpperCase())) || null;
}
...
export function resolveGpuTier({ gpuRawName, vramGb }) {
  ...
  const label =
    gpuRawName?.replace(/^NVIDIA\s+/i, "").trim() || "Unknown GPU";
  return {
    gpuModel: label,
    gpuRawName: gpuRawName || null,
